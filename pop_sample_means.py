...
for _, row in rank_distributions.iterrows():
    ...
    # Calculate the number of samples needed (rounded up to the nearest 500)
    num_samples = math.ceil(num_players / 500)

    # Take one sample for every 500 players
    mmr_values = [generate_random_mmr(min_mmr, max_mmr) for _ in range(num_samples)]

    # Create a row for each sampled MMR value set
    for mmr_sample in mmr_values:
...
sampled_df["rank_order"] = sampled_df["rank"].map(rank_order)

sampled_df.to_json("sampled_population_with_id.json", orient="records")
